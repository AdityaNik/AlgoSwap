...
export class AlgoBridgeContract extends Contract {
  ...
  public setupToken(token: Asset, dailyLimit: uint64, minAmount: uint64, maxAmount: uint64): boolean {
  ...
  public lockToken(token: Asset, amount: uint64, algorandAddress: bytes): boolean {
    ...
    assert(this.tokenDailyVolume(key).value + amount <= this.tokenDailyLimit(key).value, 'Token daily limit')
    assert(this.userDailyVolume(userKey).value + amount <= this.USER_DAILY_LIMIT, 'User daily limit')
  ...
  public mintToken(receiver: Account, amount: uint64): boolean {

    itxn
      .assetTransfer({
        xferAsset: Asset(741148638), // Hardcoded asset ID
        assetAmount: amount,
